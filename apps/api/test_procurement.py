...
def test_forex_seeding_and_update():
    ...
    update_response = client.put(
        "/forex/CNY", headers=headers, json={"rate": 11.8, "is_locked": True}
    )
...
def test_create_purchase_order_landed_cost_calculations():
    ...
    supplier = client.post(
        "/suppliers", headers=headers, json={"name": "Yiwu Toys Co", "currency": "CNY"}
    ).json()
            ...
            "items": [
                {
                    ...
                    "name": "Item A",
                    "quantity": 100,
                    "unit_cost_foreign": 10.0,
                },
                {
                    "sku": "SKU-B",
                    "name": "Item B",
                    "quantity": 200,
                    "unit_cost_foreign": 20.0,
                },
            ],
        },
    )
    ...
    assert po_data["total_landed_cost_inr"] == (150 * 100) + (
        300 * 200
    )  # 15000 + 60000 = 75000
