...
def create_order(order_in: OrderCreate, db: Session = Depends(get_db), _=allow_write):
    ...
    existing = (
        db.query(Order)
        .filter(Order.channel_order_id == order_in.channel_order_id)
        .first()
    )
    if existing:
        raise HTTPException(
    ...
    template = (
        ...
        .filter(ChannelFeeTemplate.channel_name == order_in.channel_name)
        .first()
    )
...
def update_order_status(
    order_id: str, order_in: OrderUpdate, db: Session = Depends(get_db), _=allow_write
):
