...
@router.post(
    "", response_model=OrderReturnResponse, status_code=status.HTTP_201_CREATED
)
def create_return(
    return_in: OrderReturnCreate, db: Session = Depends(get_db), _=allow_write
):
    ...
    if return_in.status == "restocked":
        handle_return_restock(
            return_in.product_id, return_in.quantity, return_in.order_id, db
        )
        db.commit()
