...
def create_purchase_order(
    po_in: PurchaseOrderCreate, db: Session = Depends(get_db), _=allow_write
):
    ...
    existing_po = (
        ...
        .filter(PurchaseOrder.po_number == po_in.po_number)
        .first()
    )
    if existing_po:
        raise HTTPException(
...
def handle_po_status_transition(
    po: PurchaseOrder, old_status: str, new_status: str, db: Session
):
...
def update_purchase_order(
    po_id: str, po_in: PurchaseOrderUpdate, db: Session = Depends(get_db), _=allow_write
):
    po = db.query(PurchaseOrder).filter(PurchaseOrder.id == po_id).first()
    if not po:
        raise HTTPException(
