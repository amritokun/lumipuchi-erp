...
import { Plus, Trash2, ChevronDown, ChevronUp, FileText, Calendar, DollarSign, ShieldAlert, CheckCircle, Package } from 'lucide-react';
...
export default function POsView({ token }: POsViewProps) {
  ...
  const handleSupplierChange = async (sId: string) => {
    ...
    if (selectedSupplier) {
      ...
      // Fetch current rate
      try {
        const res = await fetch(`${API_URL}/forex/${selectedSupplier.currency}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (res.ok) {
          const data = await res.json();
  ...
  const handleAddItemRow = () => {
    setItems([...items, { sku: '', name: '', quantity: 100, unit_cost_foreign: 10 }]);
  };
  ...
  const handleItemFieldChange = (index: number, field: keyof POItem, value: any) => {
    const updated = [...items];
    updated[index] = {
      ...updated[index],
      [field]: value
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate items
    const invalidItem = items.some(item => !item.sku || !item.name || item.quantity <= 0 || item.unit_cost_foreign <= 0);
    if (invalidItem) {
    ...
    <div className="space-y-6">
      ...
      {showAddForm && (
        <form onSubmit={handleSubmit} className="glass-panel p-6 rounded-2xl space-y-6">
          <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wider">Initialize Purchase Shipment</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-450 mb-1">PO Number *</label>
              <input
                type="text"
                required
                value={poNumber}
            ...
            <div>
              <label className="block text-xs font-medium text-slate-450 mb-1">Currency</label>
              <input
                type="text"
                disabled
                value={currency}
            ...
            <div>
              <label className="block text-xs font-medium text-slate-450 mb-1">Exchange Rate (Locked) *</label>
              <input
                type="number"
                step="0.01"
                required
          ...
          <div className="border-t border-white/5 pt-4">
            <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Landed Cost Shared Expenses (INR)</h4>
            <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
              <div>
                <label className="block text-[10px] font-medium text-slate-450 mb-1">China Domestic Shipping</label>
                <input
                  type="number"
                  value={domesticShipping}
                  onChange={(e) => setDomesticShipping(parseFloat(e.target.value) || 0)}
                  className="w-full bg-slate-900/60 border border-white/10 rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                />
              </div>
              <div>
                <label className="block text-[10px] font-medium text-slate-450 mb-1">Int'l Freight</label>
                <input
                  type="number"
                  value={internationalFreight}
                  onChange={(e) => setInternationalFreight(parseFloat(e.target.value) || 0)}
                  className="w-full bg-slate-900/60 border border-white/10 rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                />
              </div>
              <div>
                <label className="block text-[10px] font-medium text-slate-450 mb-1">Customs Duty (%)</label>
                <input
                  type="number"
                  value={customsDuty}
              ...
              <div>
                <label className="block text-[10px] font-medium text-slate-450 mb-1">Clearing Charges</label>
                <input
                  type="number"
                  value={clearingCharges}
                  onChange={(e) => setClearingCharges(parseFloat(e.target.value) || 0)}
                  className="w-full bg-slate-900/60 border border-white/10 rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                />
              </div>
              <div>
                <label className="block text-[10px] font-medium text-slate-450 mb-1">Insurance</label>
                <input
                  type="number"
                  value={insurance}
              ...
              <div>
                <label className="block text-[10px] font-medium text-slate-450 mb-1">Other Charges</label>
                <input
                  type="number"
                  value={otherCharges}
              ...
              {items.map((item, idx) => (
                <div key={idx} className="flex flex-col md:flex-row gap-3 items-end">
                  <div className="w-full md:w-1/4">
                    {idx === 0 && <label className="block text-[10px] font-medium text-slate-450 mb-1">SKU *</label>}
                    <input
                      type="text"
                      required
                      placeholder="SKU-XYZ"
                  ...
                  <div className="w-full md:w-2/5">
                    {idx === 0 && <label className="block text-[10px] font-medium text-slate-450 mb-1">Product Description *</label>}
                    <input
                      type="text"
                      required
                  ...
                  <div className="w-full md:w-1/6">
                    {idx === 0 && <label className="block text-[10px] font-medium text-slate-450 mb-1">Qty *</label>}
                    <input
                      type="number"
                      required
                      value={item.quantity}
                      onChange={(e) => handleItemFieldChange(idx, 'quantity', parseInt(e.target.value) || 0)}
                      className="w-full bg-slate-900/60 border border-white/10 rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                    />
                  </div>
                  <div className="w-full md:w-1/6">
                    {idx === 0 && <label className="block text-[10px] font-medium text-slate-450 mb-1">Unit Cost ({currency}) *</label>}
                    <input
                      type="number"
                      step="0.01"
                      required
                      value={item.unit_cost_foreign}
                      onChange={(e) => handleItemFieldChange(idx, 'unit_cost_foreign', parseFloat(e.target.value) || 0)}
                      className="w-full bg-slate-900/60 border border-white/10 rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                    />
                  </div>
                  <button
          ...
          >
            {submitting ? 'Calculating Landed Costs...' : 'Save & Calculate Landed Costs'}
          </button>
        </form>
      )}
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {pos.map((po) => (
            <div key={po.id} className="glass-card rounded-2xl overflow-hidden border border-white/5">
              {/* Summary Bar */}
              <div 
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-md font-bold text-white font-outfit">{po.po_number}</h4>
                    <span className="text-[10px] text-slate-450 block">Supplier: {po.supplier.name}</span>
                  </div>
                </div>

                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-xs">
                  <div>
                    <span className="text-[9px] uppercase text-slate-500 block">Issue Date</span>
                    <span className="text-slate-300 flex items-center gap-1 mt-0.5"><Calendar size={12} /> {new Date(po.issue_date).toLocaleDateString()}</span>
                  </div>
                  <div>
                    <span className="text-[9px] uppercase text-slate-500 block">Raw Cost</span>
                    <span className="text-slate-300 mt-0.5">{po.currency} {po.items.reduce((sum, item) => sum + (item.unit_cost_foreign * item.quantity), 0).toLocaleString()}</span>
                  </div>
                  <div>
                ...
                <div className="text-slate-450">
                  {expandedPoId === po.id ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
                </div>
              </div>
                ...
                <div className="bg-slate-950/40 border-t border-white/5 p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-xs">
                    <div className="p-3 bg-slate-900/60 rounded-xl border border-white/5">
                      <span className="text-slate-500 block">Forex Exchange Rate</span>
                      <span className="font-bold text-white mt-1 block">₹{po.exchange_rate} / {po.currency}</span>
                    </div>
                    <div className="p-3 bg-slate-900/60 rounded-xl border border-white/5">
                      <span className="text-slate-500 block">Customs Duty Rate</span>
                      <span className="font-bold text-white mt-1 block">{po.customs_duty_percent}%</span>
                    </div>
                    <div className="p-3 bg-slate-900/60 rounded-xl border border-white/5">
                      <span className="text-slate-500 block">Int'l Freight Charges</span>
                      <span className="font-bold text-white mt-1 block">₹{po.international_freight.toLocaleString()}</span>
                    </div>
                    <div className="p-3 bg-slate-900/60 rounded-xl border border-white/5">
                      <span className="text-slate-500 block">Other Shared Charges</span>
                      ...
                      <table className="w-full text-left border-collapse text-xs">
                        ...
                        <tbody className="divide-y divide-white/5 text-slate-300">
                          {po.items.map((item, idx) => {
                            const rawInr = item.unit_cost_foreign * po.exchange_rate;
