...
export default function OrdersView({ token }: OrdersViewProps) {
  ...
  const totalPayout = orders.reduce(
    (sum, o) => (o.status !== "cancelled" ? sum + o.payout_amount : sum),
    0,
  );
  const totalProfit = orders.reduce(
    (sum, o) => (o.status !== "cancelled" ? sum + o.profit_margin : sum),
    0,
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 font-medium">
            Automatic marketplace channel API data pulling and stock allocation
            reservation
          </p>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="glass-panel p-5 rounded-2xl">
              ...
              <span className="text-[9px] text-slate-500 block mt-1">
                Net receivable after commissions
              </span>
            </div>
            ...
            <div className="glass-panel p-5 rounded-2xl">
              ...
              <div className="flex items-baseline gap-1 mt-1">
                <span
                  className={`text-2xl font-extrabold ${totalProfit >= 0 ? "text-emerald-600" : "text-rose-600"}`}
                >
                  ₹{totalProfit.toLocaleString("en-IN")}
                </span>
                <span
                  className={`text-xs font-bold ${totalProfit >= 0 ? "text-emerald-500" : "text-rose-500"}`}
                >
                  ({avgMargin.toFixed(1)}%)
                </span>
              </div>
              <span className="text-[9px] text-slate-500 block mt-1">
                Profit after Landed cost & fees
              </span>
            </div>
            ...
            <div className="glass-panel p-5 rounded-2xl">
              ...
              <div className="flex items-baseline gap-1 mt-1">
                ...
                <span className="text-xs font-bold text-slate-400">
                  ({returns.length} returned)
                </span>
              </div>
              <span className="text-[9px] text-slate-500 block mt-1">
                Returned units ratio
              </span>
            </div>
          </div>
                  ...
                  <tbody className="divide-y divide-white/5 text-xs">
                    {orders.length === 0 ? (
                      <tr>
                        ...
                        >
                          No synced orders found. Click Sync above to mock!
                        </td>
                      </tr>
                    ) : (
                      orders.map((o) => (
                        <tr key={o.id} className="hover:bg-white/5 transition">
                          ...
                          <td className="p-3.5 text-center">
                            <strong className="text-slate-300">
                              ₹{o.payout_amount.toFixed(2)}
                            </strong>
                            <span className="block text-[9px] text-slate-500 mt-0.5">
                              Fees: ₹
                              {(o.selling_price - o.payout_amount).toFixed(0)}
                            </span>
                          </td>
              ...
              <div className="space-y-4">
                {returns.length === 0 ? (
                  <div className="glass-panel p-6 text-center text-slate-500 text-xs">
                    No returns filed yet. Syncing can file random returns.
                  </div>
                ) : (
                  returns.map((r) => (
                    <div
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <span className="text-[9px] text-slate-500">
                            Qty: {r.quantity} | Reason: {r.reason || "N/A"}
                          </span>
                        </div>
                        <span
                          className={`px-2 py-0.5 text-[9px] font-bold rounded uppercase border ${
