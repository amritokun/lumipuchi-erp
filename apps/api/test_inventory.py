...
def test_manual_inventory_adjustment_creates_logs():
    ...
    supplier = client.post(
        "/suppliers", headers=headers, json={"name": "Shenzhen Electrics B"}
    ).json()
    prod = client.post(
        "/products",
        headers=headers,
...
def test_po_status_transition_updates_inventory():
    ...
    # 1. Create supplier and product
    supplier = client.post(
        "/suppliers", headers=headers, json={"name": "Yiwu Toys C", "currency": "CNY"}
    ).json()
    prod = client.post(
        "/products",
        headers=headers,
    ...
    update_shipped = client.put(
        f"/purchase-orders/{po['id']}", headers=headers, json={"status": "shipped"}
    )
    ...
    update_delivered = client.put(
        f"/purchase-orders/{po['id']}", headers=headers, json={"status": "delivered"}
    )
    ...
    assert any(
        ...
        and log["quantity"] == 100
        and "PO Received" in log["reference"]
        for log in logs
    )
