...
def seed_default_templates(db: Session):
    defaults = [
        {
            ...
            "referral_fee_percent": 12.0,
            "fixed_closing_fee": 40.0,
            "weight_handling_fee": 65.0,
            "other_fees": 0.0,
            "is_default": True,
        },
        {
            "channel_name": "Amazon FBA",
            "referral_fee_percent": 15.0,
            "fixed_closing_fee": 30.0,
            "weight_handling_fee": 55.0,
            "other_fees": 0.0,
            "is_default": False,
        },
        {
    ...
    for item in defaults:
        existing = (
            db.query(ChannelFeeTemplate)
            .filter(ChannelFeeTemplate.channel_name == item["channel_name"])
            .first()
        )
        if not existing:
...
@router.post(
    "/templates",
    response_model=ChannelFeeTemplateResponse,
    status_code=status.HTTP_201_CREATED,
)
def create_template(
    template_in: ChannelFeeTemplateCreate, db: Session = Depends(get_db), _=allow_write
):
    existing = (
        db.query(ChannelFeeTemplate)
        .filter(ChannelFeeTemplate.channel_name == template_in.channel_name)
        .first()
    )
    if existing:
        raise HTTPException(
...
def update_template(
    ...
    template = (
        db.query(ChannelFeeTemplate)
        .filter(ChannelFeeTemplate.id == template_id)
        .first()
    )
    if not template:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Channel template not found"
        )
...
def calculate_pricing(
    req: PricingCalculationRequest, db: Session = Depends(get_db), _=allow_read
):
    ...
    if req.template_id:
        template = (
            db.query(ChannelFeeTemplate)
            .filter(ChannelFeeTemplate.id == req.template_id)
            .first()
        )
        if not template:
            raise HTTPException(
        ...
        if (
            req.referral_fee_percent is not None
            or req.fixed_closing_fee is not None
            or req.weight_handling_fee is not None
            or req.other_fees is not None
        ):
            ...
            default_t = (
                db.query(ChannelFeeTemplate)
                .filter(ChannelFeeTemplate.is_default == True)
                .first()
            )
            if default_t:
    ...
    net_margin_percent = (
        (net_margin_amount / req.selling_price) * 100.0
